# 10x Faster GPT Output Embedding with a Vector Index\*

\* _on CPU, GPT2, batch size = 4, approximate search, including only the output embedding matrix_
...
To generate text with these models, we sample from the model's predicted token probabilities. A popular method is [top-k sampling](https://ivibudh.medium.com/a-guide-to-controlling-llm-model-output-exploring-top-k-top-p-and-temperature-parameters-ed6a31313910), which samples from the k most probable tokens, preventing the selection of highly unlikely tokens.
...
The HNSW layer is built on top of [hnswlib](https://github.com/nmslib/hnswlib) and outputs the indices and logits of the largest k elements.
