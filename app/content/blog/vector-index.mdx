...
It performs well with the high-dimensional embeddings and offers a higher throughput than IVF. The long index build times aren't an issue for inference and performance can be tuned using the parameters ef and M.
...
Graph based vector indices aren't easily parallelizable on the GPU, so for now we focus on CPU inference. I tried the GPU index [CAGRA](https://developer.nvidia.com/blog/accelerating-vector-search-fine-tuning-gpu-index-algorithms/), but couldn't get a performance improvement for small to medium batch sizes (needs further investigation).
...
To quantify the vector index accuracy, we measure the percentage of tokens where the overlap of the exponential sum from the top-k probabilities predicted with the vector index (M=32) compared to the true top-k is under a threshold.
...
The next step is to make a hacky integration into [llama.cpp](https://github.com/ggerganov/llama.cpp) and better benchmarks.
